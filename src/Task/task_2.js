...
const submitBtn = document.querySelector('button[data-action="create"]');
...
function createBoxes(amount, size = 30) {
  const resaltBoxes = Array(Number(amount))
    ...
    .map((item, idx) => {
      ...
      const newBox = `<div style="background-color: ${backgroundColor}; width: ${
        size + idx * 10
      }px; height: ${size + idx * 10}px;"></div>`;
      return (item += newBox);
    });
